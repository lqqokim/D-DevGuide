  ...
  <div class="container-panel">
    ...
    <div class="product-manage-wrap mgb-80">
      ...
      <div class="right-proc-content">
        ...
        <!-- 제품정보 컴포넌트 -->
        <product-info ref="productInfoComp" :product="selectedProduct" />
        <!-- 스태프 목록 컴포넌트 -->
        <product-staff-list
...
import { IProduct, IFile, IStaff } from '@/store/modules/download';
...
export default class LibraryDownloadProductManage extends Vue
  implements IProductManage<IProduct> {
  @Common.Action('alert') alertAction!: (payload: IAlert) => Promise<any>;
  @Download.Action('registerProduct') registerProductAction!: (
    product: IProduct
  ) => Promise<any>;
  @Download.Action('updateProduct') updateProductAction!: (
    product: IProduct
  ) => Promise<any>;
  ...
  get products(): IProduct[] {
    ...
    return this.$store.state.download.products;
  }

  created() {
    const products: IProduct[] = this.$store.state.download.products;
  ...
  async onclickSave(): Promise<any> {
    ...
    // this.initialProducts = this.$store.state.download.products.slice();
