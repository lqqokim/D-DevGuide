<template>
  <div class="container-panel">
    <div class="view-top">
      <h2 class="tit-con-text">질문 / 답변<span>제품관리</span></h2>
      <div class="pst-rbtn">
        <button
          type="button"
          class="btn-sub-large"
...
import { IStaff, IProduct } from '@/store/modules/forum';
...
export default class ForumProductManage extends Vue
  implements IProductManage<IProduct> {
  @Common.Action('alert') alertAction!: (payload: IAlert) => Promise<any>;
  @Forum.Action('registerProduct') registerProductAction!: (
    product: IProduct
  ) => Promise<any>;
  @Forum.Action('updateProduct') updateProductAction!: (
    product: IProduct
  ) => Promise<any>;
  ...
  get products(): IProduct[] {
    ...
    return this.$store.state.forum.products;
  }

  created() {
    const products: IProduct[] = this.$store.state.forum.products;
