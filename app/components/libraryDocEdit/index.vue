...
import * as doc from '@/store/modules/document';
...
export default class LibraryDocEdit extends Vue {
  @Doc.Action('updateDoc') updateDocAction!: (payload: doc.IDocument) => void;
  ...
  redirectDocList(): void {
    this.$router.push({
      name: 'docList',
      params: {
        productCode: this.$store.state.document.selectedProduct.productCode,
      },
    });
  }
}
