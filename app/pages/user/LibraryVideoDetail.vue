<template>
  ...
  <!--            <h2 class="tit-con-text">동영상 DEWS/UI</h2>-->
  <!--          </div>-->
  ...
  <!--                width="560"-->
  <!--                height="315"-->
  <!--                src="https://www.youtube.com/embed/y1O-NQSn_dI"-->
  <!--                frameborder="0"-->
  ...
  <!--                <div class="user-info">-->
  <!--                  <span>3일전</span> <span class="hit">조회 300</span>-->
  <!--                  <span>노휘겸(플랫폼개발1팀)</span>-->
  <!--                </div>-->
  ...
  <!--                  <p>-->
  <!--                    2019년 11월 12일에 진행된 세미나에서 DEWS/UI 따라하기 세션의-->
  <!--                    영상입니다.-->
  <!--                  </p>-->
  <!--                  <p>-->
  <!--                    내부회계관리제도 운영위원회에서 공지한 내부회계관리제도 설계-->
  <!--                    및 운영에 관한 개념체계 등 관련 파일입니다.-->
  <!--                  </p>-->
  <!--                  <p>-->
  <!--                    D_ERP DICS 내부회계관리솔루션 소개영상(2)<br />-->
  <!--                    [바로보기] <br />http://reurl.kr/1AA2EC0ERF-->
  <!--                    <br />[다운로드]<br />-->
  <!--                    http://reurl.kr/1AA2EC12YP-->
  <!--                  </p>-->
  <!--                </div>-->
  <!--                &lt;!&ndash; TO  DO 시리즈 일때 : movie-aside &ndash;&gt;-->
  ...
  <!--                          <dt>-->
  <!--                            <i class="icon-new">N</i>2019-11-23 DEWS/UI 세미나-->
  <!--                            영상 제목이 긴 경우-->
  <!--                          </dt>-->
...
import LibraryVideoDetail from '@/components/libraryVideoDetail/index.vue';
...
@Component({
  ...
  async fetch({ store, params }) {
    try {
      ...
      await store.dispatch('video/productByCode', params.productCode);
      await store.dispatch('video/videoAllByProduct', params.productCode);
      // await store.dispatch('video/getVideosByProduct', {
      //   data: store.state.video.selectedProduct,
      // });

      // await store.dispatch('video/updateVideoCount', params._id);

      if (!params.video) {
        // await store.dispatch('video/updateVideoCount', params._id);
        await store.dispatch('video/videoDetailById', params._id);
        // await store.commit('countIncrement');
      } else {
        // await store.dispatch('video/updateVideoCount', params._id);
        await store.commit('video/selectedVideo', params.video);
        // await store.commit('video/countIncrement');

        if (
          store.state.video.selectedVideo.isSeries &&
          !store.state.video.selectedVideo.groupId
        ) {
          await store.commit(
            'video/selectedVideoSeries',
            store.state.video.selectedVideo.series
          );
        }
      }
    } catch (e) {
