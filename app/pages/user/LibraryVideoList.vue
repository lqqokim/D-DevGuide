<template>
  <!--  <div class="dbs-container-wrap">-->
  <!--    <div class="viewer-wrap">-->
  <!--      <div class="dbs-top-image-wrap movie">-->
  <!--        <div class="dbs-top-image">-->
  <!--          <dl>-->
  <!--            <dt>동영상</dt>-->
  <!--            <dd>-->
  <!--              동영상 서브 텍스트입니다. <br />-->
  <!--              동영상 서브 텍스트입니다 서브 텍스트입니다.-->
  <!--            </dd>-->
  <!--          </dl>-->
  <!--        </div>-->
  <!--      </div>-->
  <!--    </div>-->
  <!--    <div class="dbs-container">-->
  <!--      <h1 class="tit-con mgt-50">동영상</h1>-->
  <!--      <div class="position-wrap">-->
  <!--        <h2 class="tit-con-small">D_ERP</h2>-->
  <!--        <nuxt-link class="pst-more" to="/library/video/all">더보기</nuxt-link>-->
  <!--      </div>-->
  <!--      <ul class="thumb-list mgt-10">-->
  <!--        <li>-->
  <!--          <a href="">-->
  <!--            <div class="thumb">-->
  <!--              <img-->
  <!--                src="https://i.ytimg.com/vi/huF9H9SHAGA/hqdefault.jpg?sqp=-oaymwEZCNACELwBSFXyq4qpAwsIARUAAIhCGAFwAQ==&rs=AOn4CLD7292MGL79hY8MiMFu51WOa3MaTg"-->
  <!--              />-->
  <!--            </div>-->
  <!--          </a>-->
  <!--          <dl class="thumb-desc">-->
  <!--            <dt>더존 IT그룹 홍보CF</dt>-->
  <!--            <dd>5시간 전 <span class="name">더존</span></dd>-->
  <!--          </dl>-->
  <!--          <span class="thumb-info"-->
  <!--            ><span class="video-view"><span class="txt">298</span></span-->
  <!--            ><span class="bookmark"><span class="txt">34</span></span></span-->
  <!--          >-->
  <!--        </li>-->
  <!--        <li>-->
  <!--          <a href="">-->
  <!--            <div class="thumb">-->
  <!--              <img-->
  <!--                src="https://i.ytimg.com/vi/GIwTZ0fjz7s/hqdefault.jpg?sqp=-oaymwEZCNACELwBSFXyq4qpAwsIARUAAIhCGAFwAQ==&rs=AOn4CLB9JXtkfsb-WSRR0gOytPOaYfFS3Q"-->
  <!--              />-->
  <!--            </div>-->
  <!--          </a>-->
  <!--          <dl class="thumb-desc">-->
  <!--            <dt>D_ERP DICS 소개CF</dt>-->
  <!--            <dd>1일전 <span class="name">김더존</span></dd>-->
  <!--          </dl>-->
  <!--          <span class="thumb-info"-->
  <!--            ><span class="video-view"><span class="txt">298</span></span-->
  <!--            ><span class="bookmark"><span class="txt">34</span></span></span-->
  <!--          >-->
  <!--        </li>-->
  <!--        <li>-->
  <!--          <a href="">-->
  <!--            <div class="thumb">-->
  <!--              <img-->
  <!--                src="https://i.ytimg.com/vi/vtboZ3eH_ww/hqdefault.jpg?sqp=-oaymwEZCNACELwBSFXyq4qpAwsIARUAAIhCGAFwAQ==&rs=AOn4CLB9UDPw-1vCflwQTWWuc16TIAoO1Q"-->
  <!--              />-->
  <!--            </div>-->
  <!--          </a>-->
  <!--          <dl class="thumb-desc">-->
  <!--            <dt>기업을 위한 통합경영관리 프로그램 더존 SmartA</dt>-->
  <!--            <dd>1주일 전 <span class="name">더존</span></dd>-->
  <!--          </dl>-->
  <!--          <span class="thumb-info"-->
  <!--            ><span class="video-view"><span class="txt">30</span></span-->
  <!--            ><span class="bookmark"><span class="txt">5</span></span></span-->
  <!--          >-->
  <!--        </li>-->
  <!--        <li>-->
  <!--          <a href="">-->
  <!--            <div class="thumb">-->
  <!--              <img-->
  <!--                src="https://i.ytimg.com/vi/huF9H9SHAGA/hqdefault.jpg?sqp=-oaymwEZCNACELwBSFXyq4qpAwsIARUAAIhCGAFwAQ==&rs=AOn4CLD7292MGL79hY8MiMFu51WOa3MaTg"-->
  <!--              />-->
  <!--            </div>-->
  <!--          </a>-->
  <!--          <dl class="thumb-desc">-->
  <!--            <dt>더존 ERP iU 도입사례_ 싸이버로지텍 편</dt>-->
  <!--            <dd>한달 전 <span class="name">김더존</span></dd>-->
  <!--          </dl>-->
  <!--          <span class="thumb-info"-->
  <!--            ><span class="video-view"><span class="txt">298</span></span-->
  <!--            ><span class="bookmark"><span class="txt">34</span></span></span-->
  <!--          >-->
  <!--        </li>-->
  <!--        <li>-->
  <!--          <a href="">-->
  <!--            <div class="thumb">-->
  <!--              <img-->
  <!--                src="https://i.ytimg.com/vi/GIwTZ0fjz7s/hqdefault.jpg?sqp=-oaymwEZCNACELwBSFXyq4qpAwsIARUAAIhCGAFwAQ==&rs=AOn4CLB9JXtkfsb-WSRR0gOytPOaYfFS3Q"-->
  <!--              />-->
  <!--            </div>-->
  <!--          </a>-->
  <!--          <dl class="thumb-desc">-->
  <!--            <dt>기업을 위한 통합경영관리 프로그램 더존 SmartA</dt>-->
  <!--            <dd>1주일 전<span class="name">김더존</span></dd>-->
  <!--          </dl>-->
  <!--          <span class="thumb-info"-->
  <!--            ><span class="video-view"><span class="txt">1,234</span></span-->
  <!--            ><span class="bookmark"><span class="txt">340</span></span></span-->
  <!--          >-->
  <!--        </li>-->
  <!--      </ul>-->
  <!--      <div class="line"></div>-->
  <!--      <div class="position-wrap mgt-20">-->
  <!--        <h2 class="tit-con-small">DEWS FD</h2>-->
  <!--        <a class="pst-more">더보기</a>-->
  <!--      </div>-->
  <!--      <ul class="thumb-list mgt-10">-->
  <!--        <li>-->
  <!--          <a href="">-->
  <!--            <div class="thumb">-->
  <!--              <img-->
  <!--                src="https://i.ytimg.com/vi/huF9H9SHAGA/hqdefault.jpg?sqp=-oaymwEZCNACELwBSFXyq4qpAwsIARUAAIhCGAFwAQ==&rs=AOn4CLD7292MGL79hY8MiMFu51WOa3MaTg"-->
  <!--              />-->
  <!--            </div>-->
  <!--          </a>-->
  <!--          <dl class="thumb-desc">-->
  <!--            <dt>더존 ERP iU 도입사례_ 싸이버로지텍 편</dt>-->
  <!--            <dd>1주일 전<span class="name">김더존</span></dd>-->
  <!--          </dl>-->
  <!--          <span class="thumb-info"-->
  <!--            ><span class="video-view"><span class="txt">1,234</span></span-->
  <!--            ><span class="bookmark"><span class="txt">34</span></span></span-->
  <!--          >-->
  <!--        </li>-->
  <!--        <li>-->
  <!--          <a href="">-->
  <!--            <div class="thumb">-->
  <!--              <img-->
  <!--                src="https://i.ytimg.com/vi/huF9H9SHAGA/hqdefault.jpg?sqp=-oaymwEZCNACELwBSFXyq4qpAwsIARUAAIhCGAFwAQ==&rs=AOn4CLD7292MGL79hY8MiMFu51WOa3MaTg"-->
  <!--              />-->
  <!--            </div>-->
  <!--          </a>-->
  <!--          <dl class="thumb-desc">-->
  <!--            <dt>기업을 위한 통합경영관리 프로그램 더존 SmartA</dt>-->
  <!--            <dd>1주일 전<span class="name">김더존</span></dd>-->
  <!--          </dl>-->
  <!--          <span class="thumb-info"-->
  <!--            ><span class="video-view"><span class="txt">8</span></span-->
  <!--            ><span class="bookmark"><span class="txt">0</span></span></span-->
  <!--          >-->
  <!--        </li>-->
  <!--        <li>-->
  <!--          <a href="">-->
  <!--            <div class="thumb">-->
  <!--              <img-->
  <!--                src="https://i.ytimg.com/vi/huF9H9SHAGA/hqdefault.jpg?sqp=-oaymwEZCNACELwBSFXyq4qpAwsIARUAAIhCGAFwAQ==&rs=AOn4CLD7292MGL79hY8MiMFu51WOa3MaTg"-->
  <!--              />-->
  <!--            </div>-->
  <!--          </a>-->
  <!--          <dl class="thumb-desc">-->
  <!--            <dt>더존 IT그룹 홍보CF</dt>-->
  <!--            <dd>1주일 전<span class="name">김더존</span></dd>-->
  <!--          </dl>-->
  <!--          <span class="thumb-info"-->
  <!--            ><span class="video-view"><span class="txt">298</span></span-->
  <!--            ><span class="bookmark"><span class="txt">34</span></span></span-->
  <!--          >-->
  <!--        </li>-->
  <!--      </ul>-->
  <!--    </div>-->
  <!--    <div class="side-button"></div>-->
  <!--  </div>-->
  <LibraryVideoList></LibraryVideoList>
</template>
<script lang="ts">
import { Vue, Component } from 'nuxt-property-decorator';
import LibraryVideoList from '~/components/libraryVideoList/index.vue';

@Component({
  layout: 'TypeA',
  components: {
    LibraryVideoList,
  },
  async fetch({ params, store }) {
    try {
      // 제품에 대한 동영상 목록 조회
      await store.dispatch('video/getVideosByProduct', params);
    } catch (e) {
      console.error(e);
    }
  },
})
export default class extends Vue {}
</script>

<style lang="scss"></style>
